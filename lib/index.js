...
{
	var mixins = Array.apply([], arguments);



	if (!mixins.length) mixins = [{}, Structr.fh({})];


	var that = Structr.extend.apply(null, mixins)
	...
	if(!that.__construct.extend) {
		...
		{
			return Structr.apply(null, [that].concat(Structr.argsToArray(arguments)))
		};
	}
